...
# global_counts, counts_per_iter := BasicF4Alg(et_hat, vars, weights=""):
# out := [entries(global_counts, `pairs`)]:
# _mean := add([seq(rhs(x), x in out)])/numelems(out):
# w:= {seq(lhs(v)=lhs(v)^2, v in select(x->rhs(x)<_mean, out))}:

Bb := []:
for j from 1 to numelems(et_hat) do
...
d2p, d2c:=GetDegreeLCMPairs(Bb, et_hat, tdeg(op(vars)));
print(d2c):
...
w := {}:
for v in vars do
    if assigned(tmp[GetStateName(v)]) then
        w := {op(w), v=v^(tmp[GetStateName(v)])}
    end if:
end do;
printf("\n\nWeights:    %a\n\n", w):
